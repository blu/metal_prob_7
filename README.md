...
'But a recursive implementation starting off with one primary ray would need just one ray query for its first hit, versus N queries for N primary rays!' an alert reader could object. Indeed, but from the POV of counting paths, a recursive implementation is just an elegant way to reuse the 1st hit along N individual paths -- a form of memoization. A similar effect could be obtained from sample merging and/or caching techniques over N primary rays. Overall, any such optimisation does not change the count of vertices along individual paths. Particularly with regard to recursivity, you can think of one primary ray spawning N secondary rays as an extremely sub-optimal implementation of jittering of N primary rays, each spawning 1 secondary ray each. I.e. we cast N primary rays, and their individual hits just happened to be samey enough that we collapsed those into one hit. Unless we can *also* collapse the concluding vertices of each subsequent segment iteration in each path, in the grand scheme of things this one optimisation does not alter our path count, and thus -- our sample count.
